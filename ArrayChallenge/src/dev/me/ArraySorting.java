...
public class ArraySorting {
	...
	int [] newArray = generateRandomArrays(aLen);
	System.out.println("New: " + Arrays.toString(newArray));

	int [] copied = Arrays.copyOf(newArray, aLen);
	Arrays.sort(copied);
	System.out.println("Sorted: " + Arrays.toString(copied));

	int [] reversed = reverseArray(copied);
     ...
     */
    private static int [] reverseArray (int [] sorted) {

	int [] reversed = new int [aLen];
	int idx = (aLen - 1);
	for (int num : sorted) {
	    reversed[idx] = num;
	    idx--;
